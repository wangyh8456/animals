...
router.post('/login',async (req,res,next)=>{
    try{
        ...
        if(result===403){
            res.setjson(403,'改账号已被冻结，请联系管理员!');
            return
        }
        if(result){
...
router.get('/getrouter',async (req,res,next)=>{
    try{
        const result=await service.getRouter(req,next);
        res.setjson(200,JSON.stringify(result),'查询成功！')
        return
    }catch(e){
        next(e)
    }
